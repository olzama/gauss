...
for profile in "$directory"/*; do
  echo $profile
  delphin process --options="-1 -y --yy-rules --max-chart-megabytes=24000 --max-unpack-megabytes=24000" -g ~/delphin/GAUSS/gauss-repo/grammars/srg-mal/ace/srg.dat --select i-tokens "$profile"
done
