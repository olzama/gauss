...
if __name__ == '__main__':
    path_to_unparsed_ids = sys.argv[1] # textfile with all the items' ids
    path_to_testsuites = sys.argv[2] # folder with the testsuites from which we want to select items
    ...
    for i, ts_path in enumerate(sorted(glob.iglob(path_to_testsuites + '/**'))):
        ...
        for ii, row in enumerate(ts['item']):
            if row['i-id'] in ids:
